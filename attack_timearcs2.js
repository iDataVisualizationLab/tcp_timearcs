...
(function () {
  ...
  function render(data) {
    ...
    const enableBrushFn = () => {
      ...
      brush = d3.brush()
        ...
        .on('start brush end', function(event) {
          ...
          selectionTimeRange = selectedArcs.length > 0 ? { min: minTime, max: maxTime } : null;
    ...
    const updateSelectionDisplay = () => {
      ...
      if (selectedArcs.length > 0) {
        arcPaths.style('stroke-opacity', d => {
          const isSelected = selectedArcs.some(sel => 
            sel.sourceNode.name === d.sourceNode.name && 
            sel.targetNode.name === d.targetNode.name && 
            sel.minute === d.minute
          );
          return isSelected ? 1 : 0.15; // Dim unselected arcs more for better contrast
        })
        .attr('stroke-width', d => {
          const isSelected = selectedArcs.some(sel => 
            sel.sourceNode.name === d.sourceNode.name && 
          ...
          if (isSelected) {
            ...
            return Math.max(3, baseW < 2 ? baseW * 2.5 : baseW * 1.5); // Make selected arcs more prominent
          }
          return widthScale(Math.max(1, d.count));
        });

        // Update status with selection info including detected files
        ...
        const fileInfo = fileDetection.detected 
          ? `(${fileDetection.files.length} file${fileDetection.files.length > 1 ? 's' : ''}: ${fileDetection.files.join(', ')})` 
          : '';
        setStatus(statusEl, 
          `Selection: ${selectedArcs.length} arcs, ${selectedIps.size} IPs, ` +
          `time: ${selectionTimeRange.min}-${selectionTimeRange.max} ${fileInfo}`
        );
      } else {
    ...
    const exportBrushSelectionFn = () => {
      ...
      // Generate command line for tcp_data_loader_streaming.py
      const command = `python tcp_data_loader_streaming.py \\
  --data ${dataFilesArg} \\
  --ip-map ${datasetConfig.ipMapPath || '<IP_MAP_JSON>'} \\
  --output-dir ./tcp_data_${primaryAttack.replace(/[^a-z0-9]/gi, '_')}_selection \\
  --filter-ips "${ipList}" \\
  ...
  --flow-timeout-seconds 300`;
      ...
      showExportDialog(exportData, command);
    };
  ...
  function showExportDialog(exportData, command) {
    ...
    modal.innerHTML = `
      ...
      <div style="margin: 20px 0;">
        ...
        <textarea id="command-text" readonly style="width: 100%; height: 150px; font-family: monospace; font-size: 12px; padding: 10px; border: 1px solid #ddd; border-radius: 5px; background: #f8f9fa;">${command}</textarea>
      </div>
